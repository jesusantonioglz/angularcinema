...
export class PageVotesComponent implements OnInit {
	...
	votos(){
		...
		let valoraciones: Array<any> = this.pelicula.valoraciones;		/*	Hace referencia al atributo 'valoraciones'
																																			array de las valoraciones */

		let posicion = valoraciones.findIndex( usuario => usuario.userId === this.userId );	/*	Obtenemos el indice donde se
																																														encuentra el usuario que vota */
		let buscarUsuario = valoraciones.find( usuario => usuario.userId === this.userId );	/*	Buscamos al usuario que vota */

		let usuario = {												/*	Creamos una variable usuario para hacer
																							referencia al usuario del array valoraciones */
		...
		let promedio = this.PromedioPelicula( valoraciones );		/*	Obtenemos el promedio de las votaciones
																																con el método Promedio. */
